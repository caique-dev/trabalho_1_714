...
implementa o sistema proposto, modela analiticamente a política aleatória e
compara as três políticas por simulação.
...
Na política aleatória, cada chegada Poisson é marcada, independentemente,
com o índice do servidor escolhido, cuja probabilidade é $p_i=1/3$. Pelo
teorema da decomposição (\emph{splitting}), os eventos com cada marca formam
processos de Poisson mutuamente independentes, com taxas $p_i\lambda$.
Portanto,
\begin{equation}
 \lambda_i=p_i\lambda=\frac{\lambda}{3},\qquad i=1,2,3.
 \label{eq:splitting}
\end{equation}
Logo, cada servidor recebe um Poisson de taxa $\lambda/3$; a independência
decorre de o destino de uma chegada não alterar o das demais.
...
há descartes, partem também $\lambda T$, logo $X=\lambda$ nas três políticas.

Cada requisição consome $E[S]=1/\mu$ unidades de tempo de servidor. Portanto,
a carga agregada é $\lambda/\mu$, igual à soma das frações de tempo ocupado:
\begin{equation}
...
Um heap binário ordena os eventos por (instante, tipo, ordem de inserção).
Há \texttt{CHEGADA}, \texttt{PARTIDA} e \texttt{LOG}, este último apenas
para amostrar ocupações. Em empates temporais, a partida vem primeiro e
libera o servidor antes da nova alocação.

Cada servidor mantém uma \texttt{deque} de pares (chegada, serviço). O
primeiro item está em execução; os demais aguardam em ordem FCFS. Em uma
\texttt{CHEGADA}$(t)$, o balanceador lê as ocupações, aplica a política e
insere a requisição no fim da fila. Se ela estava vazia, inicia o serviço e
agenda a partida para $t+\text{serviço}$. Em uma \texttt{PARTIDA}$(t)$,
remove o primeiro item e registra $R=t-t_{\mathrm{chegada}}$; se houver
outro, ele inicia imediatamente e sua partida é agendada. Assim, apenas uma
requisição é executada por servidor e a ordem FCFS é preservada.
...
soma $\Delta t$ ao seu tempo ocupado. Assim,
...
que partem na janela.

\subsection{Reprodutibilidade e validação}

Geradores separados produzem chegadas, serviços e decisões de política. O
serviço é sorteado na chegada; assim, numa réplica, as políticas recebem os
mesmos instantes e demandas, mudando apenas o roteamento. Esses números
aleatórios comuns tornam a comparação pareada e reduzem sua variância.
Testes automatizados verificam RR, empates da FC, conservação,
reprodutibilidade, $E[R]\to E[S]$ em carga baixa, Lei de Little e os regimes
estável e instável. Instruções de execução estão no README.

\section{Desenho experimental}

As 15 configurações (5 valores de $\lambda$ $\times$ 3 políticas) tiveram
10 sementes, 5000 u.t.\ de duração e 500 u.t.\ de aquecimento. Para uma
...
$t$ de Student, que incorpora a incerteza de estimá-lo por $s$. O IC
bilateral de 95\% é
\begin{equation}
...
com a curva M/M/1 da aleatória, dada por (\ref{eq:er}). Como apenas a
aleatória satisfaz as hipóteses M/M/1, a curva é uma previsão exata somente
para essa política; RR e FC são comparadas a ela experimentalmente. Em todas
...
chegadas é, portanto, a soma de três exponenciais (Erlang-3), menos variável
que uma exponencial de mesma média; menos rajadas reduzem a espera. A FC usa
...
O desvio observado em $\lambda=2{,}7$ pode ser explicado pela condição inicial da simulação, que começa com as filas vazias. Para $\rho=0{,}9$, cada servidor teria em regime estacionário $E[N_i]=9$ requisições em média, de modo que o sistema parte de um estado muito abaixo do equilíbrio. Como a carga está próxima da saturação, as filas apresentam flutuações maiores e a influência do estado inicial demora mais para desaparecer. Assim, o período de aquecimento de 500 u.t. pode não ser suficiente para eliminar completamente esse efeito, fazendo com que parte da janela de medição ainda apresente filas menores que as esperadas e, consequentemente, valores de $E[N]$ e $E[R]$ ligeiramente abaixo do analítico. Para cargas menores, o nível estacionário é mais próximo de zero e esse viés é muito menos significativo.
...
Para $\lambda=3{,}3>3\mu=3$, a chegada excede a capacidade agregada; não há
regime estacionário e $E[N]$, $E[T_Q]$ e $E[R]$ crescem sem limite. Por isso,
as fórmulas estacionárias da Seção~III-B não se aplicam. Depois que os três
...
a saída satura na capacidade, não na chegada. Ao fim de 5000 u.t., $N(t)$
somou 1656, 1643 e 1630 requisições, contra 1500 previstos pela reta fluida;
as inclinações ajustadas, entre 0,326 e 0,331, ficaram próximas de 0,3
(Fig.~\ref{fig:instavel}). As três curvas quase se sobrepõem: com todos os
